...
void Data_Derived()
{
 ...
 int NP = sizeof(Proc_Name)/sizeof(Proc_Name[0]);

 for (int Nproc=0;Nproc<NP;Nproc++)
 {
  TString TnTname;
  TFile *f1 = new TFile("../../../Preselected_NTuples/Training__" + Proc_Name[Nproc] + "__.root");

   TnTname = "Training/";
   if(Proc_Name[Nproc]=="Full_Data" || Proc_Name[Nproc]=="Data_5perc")
...
//********************************** Get NTuples and Create New **********************************
  ...
  TFile *f = new TFile("NTuples_"+Proc_Name[Nproc]+"_.root","RECREATE");
  TString  Datatype = "Bkg";
  if (Proc_Name[Nproc] == "VBFHToBB_M_125" || Proc_Name[Nproc] =="GluGluHToBB_M125")
     Datatype = "Sig";
