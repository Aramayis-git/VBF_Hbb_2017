...
//***************************************************************************************
  ...
  TH1D * _BDTS  = new TH1D("BDTS0","BDTS0",100,-1,1);
  TH1D * _BDTB  = new TH1D("BDTOb","BDTOb",100,-1,1);
//***************************************************************************************
   ...
   for(int i=0;i<ntps0->GetEntries();i++)
    {
     ...
     minvbb   = My_minvbb;
    _BDTS->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),My_weight);
    }
   ...
   for(int i=0;i<ntpb[k]->GetEntries();i++)
    {
     ...
     minvbb   = MyB_minvbb[k];
    _BDTB->Fill(reader->EvaluateMVA("TMVAClassification_BDTG"),MyB_weight[k]);
    }
