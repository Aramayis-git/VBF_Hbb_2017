...
//*************************************** Pt > 20 jets[7] ***************************************
  ...
  TFile *f = new TFile("tmva_Results.root","RECREATE");
  gStyle->SetOptFile(0);
...
//  reader->AddVariable( "btg",                  &btgj_MVA);
//  reader->AddVariable( "btg_ord",              &btgj_ord_MVA);
...
//    if(ptj[k]>PT_min_B1 && btgj[k]>b_likehood_MAX1 && btgj[k] >= btg_min_B1)
      {
       ptb1   = ptj[k];
       eb1    = ej[k];
       etab1  = etaj[k];
